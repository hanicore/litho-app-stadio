...
def build_mesh(
    ...
    border_mm: float = 0.0,
    shape: str = "flat",
    sphere_diameter_mm: float = 100.0,
    sphere_wall_thickness_mm: float = 1.0,
    sphere_bottom_hole_mm: float = 10.0,
    sphere_top_hole_mm: float = 0.0,
    sphere_angular_height: float = 90.0,
    sphere_angular_width: float = 90.0,
    cylinder_diameter_mm: float = 70.0,
    cylinder_height_mm: float = 25.0,
    cylinder_thickness_mm: float = 5.0,
    cylinder_ledge_mm: float = 75.0,
):
    """
    ...
    curve_degrees : 0 = flat panel. >0 bends the panel around a cylinder,
                    ...
                    (ignored if shape="spherical")
    border_mm : optional solid raised border frame width (0 = no frame)
    shape : "flat" (default), "cylindrical", or "spherical"
    ...
    # ---- SPHERICAL LITHOPHANE (wraps image around a sphere) ----
    if shape == "spherical":
        # Map the 2D image to a specific angular band of the sphere
        radius = sphere_diameter_mm / 2.0
        
        # Convert angular height and width to radians
        height_rad = np.radians(sphere_angular_height)  # vertical band height
        width_rad = np.radians(sphere_angular_width)    # horizontal wrap angle
        
        # Horizontal: centered around z-axis (equator plane)
        # Maps image width to full width_rad rotation
        us = np.linspace(-width_rad/2, width_rad/2, cols)
        
        # Vertical: centered around equator
        # Maps image height to height_rad vertical span
        vs = np.linspace(height_rad/2, -height_rad/2, rows)
        
        for r in range(rows):
            v = vs[r]  # latitude angle
            cos_v = np.cos(v)
            sin_v = np.sin(v)
            
            for c in range(cols):
                i = idx(r, c)
                u = us[c]  # longitude angle
                cos_u = np.cos(u)
                sin_u = np.sin(u)
                t = total_thickness[r, c]
                
                # Inner surface at base radius
                # Outer surface at radius + wall thickness + local variation
                inner_r = radius
                outer_r = radius + sphere_wall_thickness_mm + t
                
                # Spherical coordinates: (x, y, z) = (r*cos(v)*cos(u), r*sin(v), r*cos(v)*sin(u))
                # This maps to a band centered at equator (y=0)
                top_verts[i] = (inner_r * cos_v * cos_u, inner_r * sin_v, inner_r * cos_v * sin_u)
                bot_verts[i] = (outer_r * cos_v * cos_u, outer_r * sin_v, outer_r * cos_v * sin_u)
    
    # ---- CYLINDRICAL LITHOPHANE (wraps around a cylinder) ----
    elif shape == "cylindrical" and curve_degrees > 0:
...
def generate_lithophane(
    ...
    """One-shot pipeline with all advanced options."""
    _MSG = {
    ...
    # For spheres, use sphere diameter to determine resolution
    if shape == "spherical":
        px_w = max(2, int(sphere_angular_width / 360.0 * sphere_diameter_mm * px_per_mm))
        px_h = max(2, int(sphere_angular_height / 180.0 * sphere_diameter_mm * px_per_mm))
    else:
    ...
    vertices, faces = build_mesh(
        thickness, width_mm, height_mm, base_thickness_mm, curve_degrees, border_mm, shape,
        sphere_diameter_mm, sphere_wall_thickness_mm, sphere_bottom_hole_mm, sphere_top_hole_mm,
        sphere_angular_height, sphere_angular_width,
        cylinder_diameter_mm, cylinder_height_mm, cylinder_thickness_mm, cylinder_ledge_mm
    )
