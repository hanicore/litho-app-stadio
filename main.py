...
from OpenGL.GL import (
    GL_AMBIENT, GL_AMBIENT_AND_DIFFUSE, GL_BLEND, GL_COLOR_BUFFER_BIT, GL_COLOR_MATERIAL, GL_CULL_FACE,
...
class MeshPreview(QOpenGLWidget):
    ...
    def set_mesh(self, vertices, faces, grid_shape, image_path, heightmap=None, spherical=False,
        ...
        self.vertices = (vertices - center) * scale
        self.faces = faces
        ...
        self.texture_coords = np.column_stack((
    ...
    def _set_transmission_texture(self, texture_map):
        ...
        thickness = self.min_thickness_mm + (1.0 - photo) * (self.max_thickness_mm - self.min_thickness_mm)
        transmission = np.exp(-1.12 * thickness)
        ...
        contrast = 0.55 + (self.contrast / 100.0) * 1.05
        glow = np.power(np.clip(transmission * exposure, 0.0, 1.0), 0.65)
        ...
        ivory = np.array((1.00, 0.86, 0.62), dtype=np.float32)
        rgb = np.clip(glow[..., None] * (0.35 * ivory + 0.65 * light), 0.0, 1.0)
        pixels = np.ascontiguousarray((rgb * 255.0).astype(np.uint8))
        self.texture_image = QImage(
            pixels.data, pixels.shape[1], pixels.shape[0], pixels.shape[1] * 3,
    ...
    def _set_lights(self):
        ...
        glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, (0.20, 0.17, 0.12, 1.0))
        glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, (18.0,))
    ...
    def _draw_triangles(self, start, end, textured):
        ...
        if use_texture:
            ...
            glDisable(GL_TEXTURE_2D)
            if not textured:
                glColor3f(0.38, 0.28, 0.16)
        glEnableClientState(GL_VERTEX_ARRAY)
        glEnableClientState(GL_NORMAL_ARRAY)
        glVertexPointer(3, GL_FLOAT, 0, self.vertices)
        glNormalPointer(GL_FLOAT, 0, self.vertex_normals)
        if use_texture:
            glEnableClientState(GL_TEXTURE_COORD_ARRAY)
            glTexCoordPointer(2, GL_FLOAT, 0, self.texture_coords)
        indices = np.ascontiguousarray(self.faces[start:end].reshape(-1), dtype=np.uint32)
